...
topics: [python]
abstract: "在缺乏特定機制去管理其複雜性下，電腦程式會變得臃長和難明。本節課會演示如可透過使用*函數*去重用代碼的某些部分和使用*模組*去分拆程式，來保持程式精簡和易於調試。"
...
在缺乏特定機制去管理其複雜性下，電腦程式會變得
臃長和難明。本節課會演示如可透過使用*函數*去重用
代碼的某些部分和使用*模組*去分拆程式，來保持程式
精簡和易於調試。能夠移除個別功能失調的模塊能有效
省時省力。
